...
import re #Cyan's addition

def len_of_longest_string(s):
    """Returns the length of the longest string, ignoring LaTeX formatting."""
    def remove_latex(text):
        """Remove LaTeX formatting to estimate actual text width correctly."""
        return re.sub(r'\\[a-zA-Z]+|\{|\}', '', text)  # Remove LaTeX commands
    
    return max(len(remove_latex(item)) for item in s)
...
def nice_string_output(d, extra_spacing=5, decimals=3):
    """ 
    Takes a dictionary d consisting of names and values to be properly formatted.
    Ensures LaTeX values are wrapped in `$...$` to enable proper rendering.
    """
    
    def wrap_latex(value):
        """Wraps LaTeX expressions in `$...$` to enable proper rendering in Matplotlib."""
    ...
    values = [wrap_latex(v) for v in values_to_string(d.values(), decimals=decimals)]
    max_values = len_of_longest_string(values)
